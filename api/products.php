<?php require_once __DIR__.'/../db.php'; header('Content-Type: application/json');
...
if ($q!=='') {
  if (ctype_digit($q)) { $sql .= " AND (p.id=? OR p.sku LIKE CONCAT('%',?,'%'))"; $params[]=(int)$q; $types.='i'; $params[]=$q; $types.='s'; }
  else { $sql .= " AND (p.name LIKE CONCAT('%',?,'%') OR p.sku LIKE CONCAT('%',?,'%'))"; $params[]=$q; $params[]=$q; $types.='ss'; }
}
