<?php
// public/admin/accounting_charge.php
    ...
    if (empty($errors)) {
        try {
            if (isset($mysqli) && $mysqli instanceof mysqli) {
                ...
                if ($before < $amount) throw new Exception('Недостаточно средств на балансе');
...
(function(){
  ...
  function fetchSearch(q){
    ...
    fetch('/mehanik/api/user_search.php?q=' + encodeURIComponent(q), { credentials: 'same-origin' })
      .then(r => r.ok ? r.json() : Promise.reject(r))
      .then(j => {
        results.innerHTML = '';
        if (!j.ok || !j.items || j.items.length === 0) { clearResults(); return; }
        for (const it of j.items) {
          const div = document.createElement('div');
          div.className = 'result-item';
          div.tabIndex = 0;
          div.textContent = '#' + it.id + ' — ' + (it.name || it.phone || '—') + ' · ' + (it.phone || '');
